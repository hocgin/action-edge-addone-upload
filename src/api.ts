...
// from: https://github.com/PlasmoHQ/edge-addons-api/blob/main/src/index.ts
// reason: import PlasmoHQ/edge-addons-api ncc error
export type Options = {
    productId: string
...
export const requiredFields = Object.keys(errorMap) as Array<keyof typeof errorMap>
...
export class EdgeAddonsAPI {
    ...
    async submit({filePath = "", notes = ""}) {
        ...
        await this.waitForUpload(uploadResp as any, accessToken as any)

        return this.publish(notes, accessToken as any)
    }
    ...
    async upload(readStream = null as any, _accessToken = null as any) {
        const accessToken = _accessToken || (await this.getAccessToken())
    ...
    async getPublishStatus(operationId: string, _accessToken = null as any) {
        const accessToken = _accessToken || (await this.getAccessToken())
        const statusEndpoint = `${this.publishEndpoint}/operations/${operationId}`

        return (await got.get(statusEndpoint, {
            headers: {
                Authorization: `Bearer ${accessToken}`
            }
