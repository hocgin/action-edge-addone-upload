...
const edge_addons_api_1 = __nccwpck_require__(7155);
function run(input) {
    const client = new edge_addons_api_1.EdgeAddonsAPI({
...
__nccwpck_require__.d(__webpack_exports__, {
  "EdgeAddonsAPI": () => (/* binding */ EdgeAddonsAPI),
  "errorMap": () => (/* binding */ errorMap),
...
var get_stream = __nccwpck_require__(6741);
...
const [major, minor] = external_node_process_namespaceObject.versions.node.split('.').map(v => Number(v));
function validateSearchParameters(searchParameters) {
    // eslint-disable-next-line guard-for-in
    for (const key in searchParameters) {
        const value = searchParameters[key];
        dist.assert.any([dist.string, dist.number, dist.boolean, dist.null_, dist.undefined], value);
    }
}
...
class Options {
    constructor(input, options, defaults) {
        ...
        dist.assert.any([dist.string, dist.urlInstance, dist.object, dist.undefined], input);
    ...
    */
    ...
    set body(value) {
        dist.assert.any([dist.string, dist.buffer, dist.nodeStream, dist.generator, dist.asyncGenerator, isFormDataLike, dist.undefined], value);
    ...
    */
    ...
    set cache(value) {
        dist.assert.any([dist.object, dist.string, dist.boolean, dist.undefined], value);
        if (value === true) {
    ...
    */
    ...
    set cacheOptions(value) {
        ...
        dist.assert.any([dist.number, dist.undefined], value.immutableMinTimeToLive);
    ...
    */
    ...
    set https(value) {
        ...
        dist.assert.any([dist.function_, dist.undefined], value.checkServerIdentity);
        dist.assert.any([dist.string, dist.object, dist.array, dist.undefined], value.certificateAuthority);
        dist.assert.any([dist.string, dist.object, dist.array, dist.undefined], value.key);
        dist.assert.any([dist.string, dist.object, dist.array, dist.undefined], value.certificate);
        dist.assert.any([dist.string, dist.undefined], value.passphrase);
        dist.assert.any([dist.string, dist.buffer, dist.array, dist.undefined], value.pfx);
        ...
        dist.assert.any([dist.string, dist.buffer, dist.array, dist.undefined], value.certificateRevocationLists);
        for (const key in value) {
    ...
    const paginateEach = (async function* (url, options) {
        ...
        while (numberOfRequests < pagination.requestLimit) {
            ...
            if (optionsToMerge === response.request.options) {
                ...
                dist.assert.any([dist.urlInstance, dist.undefined], optionsToMerge.url);
...
;// CONCATENATED MODULE: ./node_modules/@plasmohq/edge-addons-api/dist/index.js
// src/index.ts
...
var baseApiUrl = "https://api.addons.microsoftedge.microsoft.com";
var EdgeAddonsAPI = class {
  options = {};
  constructor(options) {
  ...
  async submit({ filePath = "", notes = "" }) {
    ...
    const uploadResp = await this.upload((0,external_fs_.createReadStream)(filePath), accessToken);
  ...
  async publish(notes = "", _accessToken = null) {
    ...
    const publishResp = await got_dist_source.post(this.publishEndpoint, options);
  ...
  async upload(readStream = null, _accessToken = null) {
    const accessToken = _accessToken || await this.getAccessToken();
    const uploadResp = await got_dist_source.post(this.uploadEndpoint, {
      body: readStream,
      headers: {
        Authorization: `Bearer ${accessToken}`,
  ...
  async getPublishStatus(operationId, _accessToken = null) {
    ...
    return got_dist_source.get(statusEndpoint, {
      headers: {
        Authorization: `Bearer ${accessToken}`
      }
    }).json();
  }
  async waitForUpload(operationId, _accessToken = null, retryCount = 5, pollTime = 3e3) {
    const accessToken = _accessToken || await this.getAccessToken();
    ...
    while (uploadStatus !== "Succeeded" && attempts < retryCount) {
      const statusResp = await got_dist_source.get(statusEndpoint, {
        headers: {
          Authorization: `Bearer ${accessToken}`
        }
      }).json();
      if (statusResp.status === "Failed") {
        throw new Error(statusResp.message || statusResp.errorCode + ":" + (statusResp.errors || []).join(","));
  ...
  handleTempStatus = (statusCode, action) => {
    if (statusCode !== 202) {
      if (statusCode >= 500) {
  ...
  getAccessToken = async () => {
    const data = await got_dist_source.post(`${this.options.accessTokenUrl}`, {
